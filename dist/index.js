...
function set (target, key, val) {
  ...
  if (target._isVue || (ob && ob.vmCount)) {
    process$1.env.NODE_ENV !== 'production' && warn(
...
var Watcher = function Watcher (
  ...
  this.expression = process$1.env.NODE_ENV !== 'production'
    ? expOrFn.toString()
    : '';
...
class LodgerStore extends index_esm.Store {
  ...
  static use(module, namespaced = true) {
    ...
    modules$2[key] = Object.assign({}, module[key], {
      namespaced
    });
  }
...
const _theDoc = (docs, id) => {
  ...
  if (!(doc && RxDB.isRxDocument(doc))) throw new LodgerError('no doc found %%', {
    id
  });
  return doc;
};
...
class Lodger {
  constructor(taxonomii, forms, db, store) {
    ...
    this.db = db;
    this.store = store;
    const debug = Debug('lodger:constructor'); // const subscriberData = this.subscriberData.bind(this)
  ...
  */
  ...
  subscribe(taxonomii, criteriuCerut, subscriberName = 'main') {
    ...
    if (!vueHelper.subsData[subscriberName]) {
      Vue.set(vueHelper.subsData, subscriberName, {}); // debug('D initializat subscriber: ', subscriberName)
    }
